...
const Tag = ({ label, color = K.cyan }: { label: string; color?: string }) => (
  <div style={{ fontSize:7, fontWeight: 400, letterSpacing:"0.2em",
...
const BigNum = ({ value, color, size = 30 }: { value: string; color: string; size?: number }) => (
  <div style={{ fontSize: size, fontWeight: 400, color,
...
const StatRow = ({ label, value, color }: { label: string; value: string; color: string }) => (
  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
    ...
    <span style={{ fontSize:10, fontWeight: 400, color }}>{value}</span>
  </div>
);
          ...
          <div style={{ display:"flex", justifyContent:"space-between",
            ...
            <div style={{ display:"flex", alignItems:"center", gap:14 }}>
              ...
              <div style={{ width:46, height:46, borderRadius:10,
                ...
                fontSize:16, fontWeight: 400, color:K.cyan }}>
                {studentName.split(" ").map(n=>n[0]).join("").toUpperCase().slice(0,2)}
              </div>
              <div>
                <div style={{ fontSize:7, color:K.cyan, letterSpacing:"0.2em",
                  fontWeight: 400, textTransform:"uppercase", marginBottom:3 }}>
                  ◈ Edullent AI · Weekly Report
                </div>
                <div style={{ fontSize:20, fontWeight: 400, lineHeight:1, color:K.white }}>
                  {studentName}
                </div>
            ...
            <div style={{ display:"flex", gap:12, alignItems:"center" }}>
              <div style={{ textAlign:"center" }}>
                ...
                <div style={{ fontSize:22, fontWeight: 400, color:tColor,
                  textShadow:`0 0 16px ${tColor}` }}>{tArrow}</div>
                <div style={{ fontSize:8, color:tColor, fontWeight: 400 }}>{trend.toUpperCase()}</div>
              </div>
              ...
              <div style={{ textAlign:"center" }}>
                ...
                <div style={{ fontSize:14, fontWeight: 400, color:rColor,
              ...
              <div style={{ padding:"8px 20px", borderRadius:8,
                ...
                <div style={{ fontSize:15, fontWeight: 400, color:vColor }}>{verdict}</div>
              </div>
            </div>
          </div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:5, marginTop:4 }}>
                {attPie.map(d => (
                  <div key={d.name} style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    ...
                    <span style={{ fontSize:9, fontWeight: 400, color:d.color }}>{d.value}d</span>
                  </div>
                ))}
              </div>
            </Box>
              ...
              <div style={{ display:"flex", gap:3, marginTop:8, justifyContent:"center" }}>
                {[["A+","≥90",K.green],["A","≥80",K.green],["B","≥70",K.cyan],["C","<70",K.amber]].map(([g,r,c])=>(
                  <div key={g} style={{ flex:1, background:`${c}15`, border:`1px solid ${c}30`,
                    ...
                    <div style={{ fontSize:8, fontWeight: 400, color:c as string }}>{g}</div>
            ...
            <Box style={{ width:148 }} accent={K.orange}>
              ...
              <div style={{ marginTop:6 }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                  ...
                  <span style={{ fontSize:9, fontWeight: 400, color:K.green }}>
            ...
            <Box style={{ width:195 }} accent={K.amber}>
                ...
                {tips.length > 0 ? tips.map((t: { tip: string; reason: string }, i: number) => (
                  <div key={i} style={{
                    ...
                    <div style={{ display:"flex", alignItems:"flex-start", gap:6 }}>
                      <span style={{ fontSize:7, fontWeight: 400, color:K.amber,
                        background:`${K.amber}20`, borderRadius:3,
                        padding:"2px 5px", flexShrink:0, marginTop:1 }}>
                        {String(i+1).padStart(2,"0")}
                      </span>
                      <div>
                        <div style={{ fontSize:9, fontWeight: 400, color:K.white, marginBottom:2, lineHeight:1.3 }}>
                          {t.tip}
                        </div>
              ...
              {report?.message && (
                <div style={{ marginTop:10, background:`${K.cyan}08`,
                  ...
                  <div style={{ fontSize:7, color:K.cyan, fontWeight: 400,
